...
* **Throughput**:
  * Average step duration: 32.0s.
  * True throughput: **16,376 tokens/s**.
* **Loss Convergence**:
  * Train loss converged to 4.22.
  * Validation perplexity converged to 558.30 (step 100).
* **Generation Output**:
  * Autoregressive completions showed basic Python indentation structures and English phrasing.
...
* **VRAM Footprint**:
  * Successfully trained with micro-batch size 4 (no VRAM OOM). Peak VRAM dropped to ~6.4 GB (a **18% VRAM reduction** despite processing 4× larger micro-batches).
* **Throughput**:
...
* **Loss Convergence**:
  * Final training loss: **4.1813** (lower than baseline's 4.2195).
  * Final validation perplexity: **536.26** (lower than baseline's 558.30).
* **Generation Output**:
  * The completions produced more logical coding block comments (e.g. autocompleting Python array comments) and clean structured Javascript loops.
